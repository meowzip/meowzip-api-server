...
public interface ProfileSwagger {
    ...
    CommonListResponseV2<PostResponseDTO> showPostsByWriter(Principal principal,
                                                            @Parameter(in = ParameterIn.QUERY, name = "member-id") Long memberId,
                                                            PageRequest pageRequest);

    @Operation(summary = "북마크한 게시글 조회")
    CommonListResponseV2<PostResponseDTO> showBookmarkedPosts(Principal principal,
                                                            PageRequest pageRequest);
}
